...
void loop(){
  ...
  if(error == 0){
    ...
    Serial.println(F("You can now calibrate the esc's and upload the ESC CALIBRATION code."));
  }
  else{
   Serial.println(F("The setup is aborted due to an error."));
   Serial.println(F(""));
   Serial.println(F("IF ANY PROBLEM REPEAT THE PROCEDURE"));
  }
  while(1);
}
...
void intro(){
  ...
  Serial.println(F("ALWAYS REMOVE THE PROPELLERS TILL THE LAST PROCEDURE"));
}
